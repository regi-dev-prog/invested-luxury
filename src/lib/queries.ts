...
export async function getArticlesBySubCategory(categorySlug: string) {
  const query = `*[_type == "article" && status == "published" && "${categorySlug}" in categories[]->slug.current] | order(coalesce(publishedAt, _createdAt) desc) {
    ${articleFields}
  }`
...
export async function getArticlesByParentCategory(parentSlug: string) {
  ...
  const query = `*[_type == "article" && status == "published" && count(categories[slug.current in [${slugArray}]]) > 0] | order(coalesce(publishedAt, _createdAt) desc) {
    ${articleFields}
  }`
...
export async function getArticlesByParentCategoryAlt(parentSlug: string) {
  ...
  const query = `*[_type == "article" && status == "published" && (${conditions})] | order(coalesce(publishedAt, _createdAt) desc) {
    ${articleFields}
  }`
...
export async function getArticleCountsByParentCategory(parentSlug: string) {
  ...
  for (const slug of subSlugs) {
    const query = `count(*[_type == "article" && status == "published" && "${slug}" in categories[]->slug.current])`
