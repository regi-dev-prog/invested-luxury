...
const res = await fetch(`https://4b3ap7pf.api.sanity.io/v2021-06-07/data/query/production?query=*[_type=="article"&&!(_id in path("drafts.**"))]{title,"slug":slug.current,"categories":categories[]->name,"mainImage":mainImage.asset->url,"categorySlug":categories[0]->slug.current,"parentCategory":categories[0]->parentCategory}|order(_createdAt desc)`);
...
for (const article of result) {
  ...
  console.log(`${data.id ? '✅' : '❌'} ${article.title} → ${articleUrl}`);
